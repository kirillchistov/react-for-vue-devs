...
describe('App — базовые маршруты', () => {
  ...
  it('неавторизованного пользователя "/profile" перенаправляет на /login', () => {
    renderWithProviders(<App />, { route: '/profile' });
    expect(screen.getByRole('heading', { name: 'Вход' })).toBeInTheDocument();
  });

  it('авторизованному пользователю "/profile" показывает личный кабинет', () => {
    renderWithProviders(<App />, { route: '/profile', user: { email: 'pilot@stellar.burger' } });
...
describe('App — авторизация', () => {
  it('после входа со страницы /login видно личный кабинет и email в шапке', () => {
  ...
  it('"Выйти" в личном кабинете убирает email из шапки и возвращает на главную', () => {
    renderWithProviders(<App />, { route: '/profile', user: { email: 'pilot@stellar.burger' } });

    fireEvent.click(screen.getByRole('button', { name: /выйти/i }));

    expect(screen.getByRole('heading', { name: 'Конструктор Stellar Burger' })).toBeInTheDocument();
