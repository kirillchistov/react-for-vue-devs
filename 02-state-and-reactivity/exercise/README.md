# Упражнение: конструктор бургера

Полное описание задачи и теория — в [README модуля](../README.md).

```sh
npm install        # один раз, из корня репозитория (npm workspaces)
npm test            # красный — TODO ещё не сделаны
npm run test:watch  # тот же прогон, но в watch-режиме
npm run dev          # http://localhost:5173
```

Файлы с `// TODO`:

- `src/components/IngredientCard.tsx` — кнопка «Добавить»
- `src/components/BurgerConstructorItem.tsx`
- `src/components/BurgerConstructor.tsx`
- `src/App.tsx` — `handleAdd` / `handleRemove`
