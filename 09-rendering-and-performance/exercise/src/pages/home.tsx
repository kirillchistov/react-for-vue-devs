...
export function HomePage() {
  ...
  // TODO: оберните эту функцию в useCallback (deps: [dispatch]), чтобы она не
  // создавалась заново при каждом рендере HomePage — иначе IngredientCard,
  // обёрнутый в memo(...), получает новый onAdd на каждый ре-рендер родителя
  // (например, из-за debounce-таймера) и мемоизация не спасает от перерисовки.
  const handleAdd = (ingredient: (typeof ingredients)[number]) => dispatch(addItem(ingredient));
    ...
    <>
      ...
      <div className={styles.layout}>
        <div className={styles.catalogColumn}>
          ...
          <IngredientCatalog ingredients={visibleIngredients} status={ingredientsState.status} onAdd={handleAdd} />
        </div>
